...
class TaxonomyCallbacks{
    ...
    public function taxSectionManager(){
        echo 'Crie as suas taxonomias!';
    }

    public function taxSanitize($input){
        $output = get_option('goweb_plugin_tax'); 
    ...
    $checked = false;
      ...
      if(isset($_POST["edit_taxonomy"])){
        ...
        '[' . $name . ']" value="1" class="" ' . ($checked ? 'checked' : '') . '>
  ...
  public function checkboxPostTypesField($args){     
    ...
    if(isset($_POST["edit_taxonomy"])){
      ...
      foreach($post_types as $post){
        ...
        $output .= '<div class="' . $classes . ' mb-10"><input type="checkbox" id="' . $post . '" name="' . $option_name . 
        '[' . $name . ']['. $post .']" value="1" class="" ' . ($checked ? 'checked' : '') . '>
        <label for="' . $post . '"><div></div></label><strong>' . $post . '</strong></div>';
      }
